...
if __name__ == '__main__':
    try:
        ...
        agente = AgenteQLearning(hiloLineas.getNumEstados(),5,hiloLineas,rA,gamma) 
        entorno = Entorno(agente, controlRobot, hiloLineas)

        #variablesGlobales.control.setHiloControl(hiloControl)

        agente.setHiloControl(entorno)

        #Cargar la tabla si se ha especificado una
        if path != None:
            
            agente.cargarTablaQ(path)
